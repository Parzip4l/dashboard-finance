...
function mapRowToData(row, isTotalRow = false) {
  // Penyerapan Total: Kolom BE (index 56)
  const penyerapanTotal = Number(row[56]) || 0;

  // Penyerapan %: Ambil LANGSUNG dari Excel kolom BF (index 57)
  // Graph API return desimal (misal 0.0747 = 7.47%), konversi ke persen
  const penyerapanPersen = (Number(row[57]) || 0) * 100;
  ...
  return {
    ...
    // Penyerapan Total: Kolom BE (index 56)
    penyerapan_total: penyerapanTotal,

    // Penyerapan %: Langsung dari Excel kolom BF (index 57)
    penyerapan_persen: penyerapanPersen,

    // Sisa Anggaran Total: Kolom BI (index 60)
...
function transformExcelToDashboardData(rows) {
  ...
  // Hitung penyerapan_persen per kategori dari total penyerapan + sisa_anggaran
  // (karena anggaran_realokasi di row[10] belum tentu akurat)
  Object.keys(categoryTotals).forEach(key => {
    const t = categoryTotals[key];
    const totalAnggaran = t.penyerapan_total + t.sisa_anggaran_total;
    t.penyerapan_persen = totalAnggaran > 0 ? (t.penyerapan_total / totalAnggaran) * 100 : 0;
  });

  const calculatedGrandTotal = createZeroTotal();
  Object.values(categoryTotals).forEach(cat => addToTotal(calculatedGrandTotal, cat));
  const grandTotalAnggaran = calculatedGrandTotal.penyerapan_total + calculatedGrandTotal.sisa_anggaran_total;
  calculatedGrandTotal.penyerapan_persen = grandTotalAnggaran > 0
    ? (calculatedGrandTotal.penyerapan_total / grandTotalAnggaran) * 100 : 0;
...
app.get('/api/debug-columns', async (req, res) => {
  try {
    ...
    const debugInfo = {
      ...
      total_cols_row0: rawData[0]?.length,
      rows_analysis: []
    };
    ...
    rawData.forEach((row, idx) => {
      ...
      if (isCategory || isCategoryCol0 || isDeptCode || isTotalLRTJ || hasPenyerapanKeyword || idx < 10) {
        debugInfo.rows_analysis.push({
          ...
          is_category_col1: isCategory,
          is_category_col0: isCategoryCol0,
          is_dept_code: isDeptCode,
          is_total_lrtj: isTotalLRTJ,
          has_penyerapan_keyword: hasPenyerapanKeyword,
          // Kolom kunci
          "row[10]_anggaran_realokasi_total": row[10],
          "row[23]_realisasi_capex": row[23],
          "row[37]_opex_verifikasi": row[37],
          "row[50]_opex_ppa": row[50],
          "row[53]_proforma": row[53],
          "row[54]_BC": row[54],
          "row[55]_BD": row[55],
          "row[56]_BE_penyerapan_total": row[56],
          "row[57]_BF": row[57],
          "row[58]_BG": row[58],
          "row[59]_BH": row[59],
          "row[60]_BI_sisa_anggaran": row[60],
          "row[61]_BJ": row[61],
        });
      }
    });
